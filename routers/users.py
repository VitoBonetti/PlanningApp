...
def delete_user(user_id: str, request: Request, background_tasks: BackgroundTasks, current_user: dict = Depends(require_admin), cursor = Depends(get_db_cursor)):
...
@limiter.limit("10/minute")
def update_user(user_id: str, u: UserUpdate, request: Request, background_tasks: BackgroundTasks, current_user: dict = Depends(require_admin), cursor = Depends(get_db_cursor)):
    if u.role == 'read_only':
        u.base_capacity = 0.0
...
@limiter.limit("5/minute")
def admin_reset_password(user_id: str, request: Request, background_tasks: BackgroundTasks, current_user: dict = Depends(require_admin), cursor = Depends(get_db_cursor)):
    reset_token = str(uuid.uuid4())
    expires = datetime.now(timezone.utc) + timedelta(hours=2)  # Shorter expiry for resets
...
@limiter.limit("5/minute")
def change_own_password(p: PasswordChange, request: Request, background_tasks: BackgroundTasks, current_user: dict = Depends(get_current_user), cursor = Depends(get_db_cursor)):
...
@limiter.limit("5/minute")
def setup_user_password(payload: UserSetupPassword, request: Request, background_tasks: BackgroundTasks, cursor = Depends(get_db_cursor)):
    ...
    cursor.execute(
        ...
        (payload.token, datetime.now(timezone.utc))
    )
...
@limiter.limit("5/minute")
def get_user_setup_info(request: Request, token: str, cursor = Depends(get_db_cursor)):
